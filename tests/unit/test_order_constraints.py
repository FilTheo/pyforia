...
def test_custom_multi_supplier_engine_applies_constraints_to_each_decision():
    class TwoSupplierEngine(SimulationEngine):
    ...
    demand = pd.DataFrame({
        "unique_id": ["A"], "period": [0],
        "date": [pd.Timestamp("2025-01-02")], "y": [0.0],
    })
    result = TwoSupplierEngine().run(
        FixedOrderPolicy(
            7.0, lead_time=1, review_period=1, service_level=0.95,
            allow_backorders=False,
        ),
        ...
        settlement_periods=0,
        order_during_settlement=False,
        demand_source_name="multi_supplier_constraint_test",
        random_seed=None,
        order_constraints=OrderingConstraints([
            MaximumOrderQuantity(5.0, mode="adjust"),
        ]),
    )
    event = result.to_event_frame().iloc[0]
    assert event["requested_order_quantity"] == 14.0
    assert event["constrained_order_quantity"] == 10.0
